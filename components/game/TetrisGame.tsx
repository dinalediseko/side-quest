...
export default function TetrisGame() {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      const { tetrisConfig } = await import("@/phaser/tetris/config");

      if (destroyed || gameRef.current) {
        return;
      }

      gameRef.current = new Phaser.Game(tetrisConfig);
    }
  ...
  useEffect(() => {
    async function scoreListener(event: Event) {
      ...
      try {
        ...
        await submitScore(currentUser.uid, username, "tetris", score);
      } catch (error) {
        console.error("Failed to save Tetris score:", error);
      }
    }

    window.addEventListener("tetris-score", scoreListener);

    return () => {
      window.removeEventListener("tetris-score", scoreListener);
    };
  }, []);
